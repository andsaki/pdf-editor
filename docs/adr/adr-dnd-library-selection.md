...
| **アクセシビリティ** | ◯ 手動実装可能<br>（Tab移動、矢印キー、ARIA属性）<br>工数: 1-2日 | ◎ 標準搭載<br>（KeyboardSensor、自動ARIA設定） |
| **画面上の操作性** | ◎ 非常に直感的 | ◯ センサー設定が必要 |
| **将来的拡張性** | △ 限定的<br>（複数選択は実装困難） | ◎ 非常に高い<br>（複数選択・グリッドスナップ等） |
| **学習コスト** | 低（APIがシンプル） | 高（概念理解が必要） |
| **実装コスト（基本）** | 低（10分） | 高（1.5時間：リサイズ自前） |
| **バンドルサイズ** | 約20kb | 約10kb |
...
react-rnd は標準的な React コンポーネントであり、以下のプロパティを受け入れるため、**基本的なアクセシビリティ機能は全て実装可能**：
